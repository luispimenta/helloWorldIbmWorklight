require(["dojo/dom",
	...
	addService("Capture", function(){
		...
		this.audioFiles = [];
		this.videoFiles = [];
		this.video = dom.byId("sim_capture_videoPlayer");
		...
		this.playVideo = function(f){
			...
			if (has("chrome") || hasFF)
				...
				f = f + ".mp4";
			if (!this.video) {
				var captureVideoContainer = dom.byId("sim_capture_videoContainer");
		...
		this.generateError = function(checked){
			if (checked == true) {
				dijit.byId('sim_capture_audioFile1').set('checked', false);
				dijit.byId('sim_capture_audioFile2').set('checked', false);
				dijit.byId('sim_capture_audioFile3').set('checked', false);
				dijit.byId('sim_capture_videoFile1').set('checked', false);
				dijit.byId('sim_capture_videoFile2').set('checked', false);
				dijit.byId('sim_capture_videoFile3').set('checked', false);
			}
		};
		...
		// Initialization
		{
			...
			if (!(has("chrome") || hasFF || has("ie") || has("safari"))) {
				...
				var captureAudioPlayer1 = dom.byId("sim_capture_audioPlayer1");
				...
				var captureAudioPlayer2 = dom.byId("sim_capture_audioPlayer2");
				...
				var captureAudioPlayer3 = dom.byId("sim_capture_audioPlayer3");
