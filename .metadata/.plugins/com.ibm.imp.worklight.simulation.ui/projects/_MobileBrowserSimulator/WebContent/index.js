			     		...
			     		dojo.forEach(response['platforms'], function(platform){ //fetch platforms from device list
			    		  ...
			    		  dojo.forEach(response['devices'], function(device){
			    		    	if ((platformID == device.platformID) || (platformID == null)) {
			    				    ...
			    				    var agentID_Worklight = device.agentID + "/Worklight/6.3";
			    				    var deviceItem = {
			    				        "type" : "device",
			    				        "id" : device.name,
			    				        "name" : device.name,
...
//				initWithoutMap);
 ...
 /*	var wireQRCode = function() {
	...
	ready(function(){		
		...
		if (has("chrome")) {
			dojo.html.set('_extensionInstallInstructionsBrowserSpecific', 
							'<p>'
			...
			dojo.connect(useragentSwitchingBtn, 'onClick', function(event){
				if (useragentSwitchingBtn.checked) {
					if (communicator.isSwitchingInstalled()) {
						...
						if (has("chrome")) {
							dojo.attr(link, 'href', "https://chrome.google.com/webstore/detail/mobile-browser-simulator/dcljefehlpjmoefgoohaobehbenocmkg");
